...
const InsuranceClaims: React.FC = () => {
  ...
  const [formData, setFormData] = useState<InsuranceClaimFormData>({
    ...
    insuranceProvider: '',
    policyNumber: '',
    claimAmount: null, // null instead of 0
    notes: ''
  });
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(claim => 
        (claim.insurance_provider?.toLowerCase().includes(query) || false) ||
        (claim.claim_reference?.toLowerCase().includes(query) || false) ||
        (claim.invoice_description?.toLowerCase().includes(query) || false)
      );
    }
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch claims
      const claimsData = await insuranceService.getPatientClaims(user?.id as number);
      ...
      const eligibleInvoices = invoicesData.filter((invoice: any) => 
  ...
  const handleUpdateClaim = async () => {
    ...
    try {
      ...
      toast.error('Failed to update insurance claim');
    }
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const response = await api.post('/insurance/documents/upload', formData, {
          headers: {
        ...
        if (response.data) {
          ...
          if (url) {
            ...
            toast.warning(`Upload succeeded but URL is missing for ${file.name}`);
          }
        }
  ...
  const getStatusBadge = (status: string) => {
    switch (status.toLowerCase()) {
      case 'draft':
        return <span className="bg-gray-200/20 text-gray-300 px-2 py-1 rounded-full text-xs">Draft</span>;
      case 'submitted':
        return <span className="bg-blue-500/20 text-blue-400 px-2 py-1 rounded-full text-xs">Submitted</span>;
      case 'approved':
        return <span className="bg-green-500/20 text-green-400 px-2 py-1 rounded-full text-xs">Approved</span>;
      case 'rejected':
        return <span className="bg-red-500/20 text-red-400 px-2 py-1 rounded-full text-xs">Rejected</span>;
      case 'reimbursed':
        return <span className="bg-purple-500/20 text-purple-400 px-2 py-1 rounded-full text-xs">Reimbursed</span>;
      default:
        return <span className="bg-gray-200/20 text-gray-300 px-2 py-1 rounded-full text-xs">{status}</span>;
    }
  };
      ...
      <Card className="p-6">
        <div className="flex flex-col md:flex-row justify-between mb-6 space-y-4 md:space-y-0">
          ...
          <div className="flex gap-2">
            {['all', 'draft', 'submitted', 'approved', 'rejected', 'reimbursed'].map((filter) => (
              <button
                key={filter}
                ...
                {filteredClaims.map((claim) => (
                  <tr key={claim.id} className="hover:bg-white/5">
                    <td className="px-4 py-4 whitespace-nowrap text-white/90">
                      ...
                      <div className="text-xs text-white/60">${parseFloat(String(claim.invoice_amount || 0)).toFixed(2)}</div>
                    </td>
                    <td className="px-4 py-4 whitespace-nowrap">
                      <div className="text-sm text-white/90">{claim.insurance_provider}</div>
                      <div className="text-xs text-white/60">Policy: {claim.policy_number}</div>
                    </td>
                    <td className="px-4 py-4 whitespace-nowrap text-white/90">
                      ...
                      <div className="flex justify-end space-x-2">
                        {claim.status === 'draft' && (
                          <>
                            ...
                            <Button variant="ghost" size="sm" onClick={() => openDeleteModal(claim)} title="Delete Claim">
                              <Trash className="w-4 h-4" />
                            </Button>
                            <Button variant="ghost" size="sm" onClick={() => openSubmitModal(claim)} title="Submit Claim">
      ...
      {isCreateModalOpen && (
        ...
        >
          <div className="space-y-4">
            <div>
              <label className="block text-white/60 text-sm mb-1">Invoice</label>
              <select
            ...
            <div>
              <label className="block text-white/60 text-sm mb-1">Insurance Provider</label>
              <input
                type="text"
                className="w-full bg-slate-800 border border-white/10 rounded-lg p-2 text-white focus:outline-none"
            ...
            <div>
              <label className="block text-white/60 text-sm mb-1">Policy Number</label>
              <input
                type="text"
                className="w-full bg-slate-800 border border-white/10 rounded-lg p-2 text-white focus:outline-none"
            ...
            <div>
              <label className="block text-white/60 text-sm mb-1">Claim Amount ($)</label>
              <input
                type="number"
                min="0.01"
                step="0.01" 
                className="w-full bg-slate-800 border border-white/10 rounded-lg p-2 text-white focus:outline-none"
                placeholder="0.00"
                value={formData.claimAmount === null ? '' : formData.claimAmount}
                onChange={(e) => {
                  if (e.target.value === '') {
                    setFormData({...formData, claimAmount: null});
                  } else {
                    const value = parseFloat(e.target.value);
                    setFormData({...formData, claimAmount: isNaN(value) ? null : value});
                  }
                }}
              />
            </div>
            ...
            <div className="flex items-start p-4 bg-red-500/10 rounded-lg border border-red-500/20">
              ...
              <div>
                ...
                <p className="text-sm text-white/70">
                  This action cannot be undone. All information associated with this claim will be permanently removed.
                </p>
              </div>
            </div>
          ...
          <div className="space-y-4">
            <div className="bg-blue-500/10 p-4 rounded-lg border border-blue-500/20">
              <p className="text-white/90 text-sm">
                ...
                This typically includes your medical invoice, receipts, 
                and any additional documentation your insurance provider requires.
              </p>
            </div>
            
            <div>
              <label className="block text-white/60 text-sm mb-3">Upload Documents</label>
            ...
            {documentUrls.length > 0 && (
              <div>
                <label className="block text-white/60 text-sm mb-2">Uploaded Documents</label>
            ...
            <div className="pt-4 border-t border-white/10 flex justify-end space-x-3">
              ...
              <Button 
                ...
                disabled={documentUrls.length === 0 || uploadingDocuments || uploadingFiles}
              >
                {uploadingDocuments ? (
                  <>
